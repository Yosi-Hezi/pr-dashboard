...
class PrDataStore:
    ...
    async def refresh_all(
        ...
        async with self._lock:
            ...
            async def _refresh_one_gh(pr: dict, gh: GhClient) -> dict:
                try:
                    owner_repo = pr.get("repoUrl", "").replace("https://github.com/", "")
                    if not owner_repo:
                        return pr
                    gh_pr = await gh.get_pr(owner_repo, pr["id"])
                    return await gh.enrich_pr(gh_pr, role=pr.get("role", "author"))
                except GhApiError as exc:
                    log.warning("Failed to refresh GitHub PR #%d: %s", pr["id"], exc)
                    return pr
            ...
            for src, src_prs in by_source.items():
                if src.startswith("ado/"):
                    ...
                    try:
                        entries = await asyncio.gather(
                            *(_refresh_one_gh(pr, gh) for pr in src_prs)
                        )
                        new_entries.extend(entries)
                    finally:
                        await gh.close()
